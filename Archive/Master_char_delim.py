...
'''
# I decided to update the below commented code and replaced the hard code of '171620-ST-' with a varable instead.
for root, dirs, files in os.walk(zeman_folder_path):
    for file in files:
        file_path = os.path.join(root, file)
        if file.endswith(('.nc', '.Xml')) and os.path.isfile(file_path):
            with open(file_path, 'r') as f:
                content = f.read()
            # if 171620-ST- is there, remove 171620-ST- then, rename all files to exclude 171620-ST-
            if b'171620-ST-' in content.encode('utf-8'):
                new_content = content.encode('utf-8').replace(b'171620-ST-', b'')
                with open(file_path, 'w') as f:
                    f.write(new_content.decode('utf-8'))
                new_file_path = os.path.join(root, file.replace('171620-ST-', ''))
                os.rename(file_path, new_file_path)
'''
